...
public class FilterOutliers
{
	...
	public static void main_single_channel( final String[] args ) throws Exception
	{
		...
		for ( int i = 0; i < shifts.size(); i++ )
		{
			...
			for ( int d = 0; d < 3; d++ )
				diff[d] = (shifts.get(i).getSubTilePair().getFullTilePair().getB().getStagePosition( d ) - shifts.get(i).getSubTilePair().getFullTilePair().getA().getStagePosition( d )) - shifts.get(i).getOffset( d );
	...
	public static void main/*_channels_discrepancy*/( final String[] args ) throws Exception
	{
		...
		for ( int i = 0; i < shifts[0].size(); i++ )
		{
			...
			for ( int j = 0; j < 2; j++ )
				for ( int d = 0; d < 3; d++ )
					diff[j][d] = (shifts[j].get(i).getSubTilePair().getFullTilePair().getB().getStagePosition( d ) - shifts[j].get(i).getSubTilePair().getFullTilePair().getA().getStagePosition( d )) - shifts[j].get(i).getOffset( d );
