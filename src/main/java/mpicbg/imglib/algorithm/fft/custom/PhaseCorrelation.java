...
package mpicbg.imglib.algorithm.fft.custom;
...
public class PhaseCorrelation<T extends RealType<T>, S extends RealType<S>> implements MultiThreaded, Algorithm, Benchmark
{
	...
	public boolean process()
	{
		...
		final FourierTransform<T, ComplexFloatType> fft1 = new FourierTransform<>( image1, new ComplexFloatType() );
		final FourierTransform<S, ComplexFloatType> fft2 = new FourierTransform<>( image2, new ComplexFloatType() );
		...
		//
		final InverseFourierTransform<FloatType, ComplexFloatType> invFFT = new InverseFourierTransform<>( fftImage1, fft1, new FloatType() );
		invFFT.setInPlaceTransform( true );
		invFFT.setCropBackToOriginalSize( false );
		invFFT.setNumThreads( getNumThreads() );
		if ( numThreads > 0 )
			System.out.println( "### PhaseCorrelation: using " + getNumThreads() + " threads ###" );
	...
	protected void verifyWithCrossCorrelation( final ArrayList<PhaseCorrelationPeak> peakList, final int[] dimInvPCM, final Image<T> image1, final Image<S> image2 )
	{
		...
		final ArrayList<PhaseCorrelationPeak> newPeakList = new ArrayList<>();
	...
	{
		final ArrayList<PhaseCorrelationPeak> peakList = new ArrayList<>();
			...
			if ( isMax )
			{
				...
				if ( value > lowestValue )
				{
					...
					if ( offsetValidator != null )
					{
						...
						// get all the different possiblities
						//
						boolean peakFallsIntoConfidenceInterval = false;

						for ( int i = 0; i < coordinates.length; ++i )
						{
							final boolean[] currentPossiblity = coordinates[ i ];

							final int[] peakPosition = position.clone();
							...
							if ( fallsIntoConfidenceInterval( peakPosition ) )
							{
								peakFallsIntoConfidenceInterval = true;
								break;
							}
						}

						if ( !peakFallsIntoConfidenceInterval )
							continue;
					}
		...
		System.out.println( "Found " + peaksFound + " phase correlation peaks" );
