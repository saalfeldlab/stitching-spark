...
public class PairwiseStitchingParameters
{
	...
	public PairwiseStitchingParameters(final double minOverlap, final int peaksToCheck, final boolean doSubpixel, final boolean interpolateCrossCorrelation, final boolean showExpertGrouping)
	{
	...
	public static PairwiseStitchingParameters getParametersFromGD(final GenericDialog gd)
	{
		...
		final int peaksToCheck  = (int) gd.getNextNumber();
		final double minOverlap =  Math.min( Math.max( gd.getNextNumber()/100 , 0), 1);
		final boolean doSubpixel = gd.getNextBoolean();
		final boolean interpolateSubpixel = gd.getNextBoolean();
		final boolean showExpertGrouping = gd.getNextBoolean();
