...
public class PhaseCorrelationPeak2 {
	...
	public void setPcmLocation(final Localizable pcmLocation) {
	...
	public void setShift(final Localizable shift) {
		this.shift = shift;
	}
	...
	public void setSubpixelPcmLocation(final RealLocalizable subpixelPcmLocation) {
	...
	public void setSubpixelShift(final RealLocalizable subpixelShift) {
	...
	public void setPhaseCorr(final double phaseCorr) {
	...
	public void setCrossCorr(final double crossCorr) {
	...
	public void setnPixel(final long nPixel) {
		this.nPixel = nPixel;
	}
	 ...
	 * @param phaseCorr
	 */
	public PhaseCorrelationPeak2(final Localizable pcmPosition, final double phaseCorr)
	{
	 ...
	 * @param src
	 */
	public PhaseCorrelationPeak2(final PhaseCorrelationPeak2 src){
	...
	public static class ComparatorByPhaseCorrelation implements Comparator<PhaseCorrelationPeak2> {
		@Override
		public int compare(final PhaseCorrelationPeak2 o1, final PhaseCorrelationPeak2 o2) {
	...
	public static class ComparatorByCrossCorrelation implements Comparator<PhaseCorrelationPeak2>{
		@Override
		public int compare(final PhaseCorrelationPeak2 o1, final PhaseCorrelationPeak2 o2) {
			final int ccCompare = Double.compare(o1.crossCorr, o2.crossCorr);
			if (ccCompare != 0){
				return ccCompare;
			} else {
	...
	public <T extends RealType<T>, S extends RealType<S>> void calculateCrossCorr(final RandomAccessibleInterval<T> img1, final RandomAccessibleInterval<S> img2,
			final long minOverlapPx)
	{
	...
	public <T extends RealType<T>, S extends RealType<S>> void calculateCrossCorr(final RandomAccessibleInterval<T> img1, RandomAccessibleInterval<S> img2,
			final long minOverlapPx, final boolean interpolateSubpixel)
	{
		final Pair<Interval, Interval> intervals = PhaseCorrelation2Util.getOverlapIntervals(img1, img2, shift);
		...
		{
			final RealRandomAccessible< S > rra = Views.interpolate( Views.extendMirrorSingle( img2 ), new NLinearInterpolatorFactory< S >() );
	 ...
	 * @param pcm
	 */
	public <T extends RealType<T>> void  calculateSubpixelLocalization(final RandomAccessibleInterval<T> pcm){
		...
		final boolean[] allowedToMoveInDim = new boolean[ n ];
		...
		final List<RefinedPeak<Point>> res = SubpixelLocalization.refinePeaks(peaks, Views.extendPeriodic( pcm ), null, true,
	...
	public static void main(final String[] args) {
		...
		final double o1 = 6;
		final double o2 = Double.NEGATIVE_INFINITY;
		final int np1 = 30;
		final int np2 = 20;

		System.out.println( Double.isInfinite( o2 ));

		final int ccCompare = Double.compare(o1, o2);
		if (ccCompare != 0)
		...
		System.exit( 0 );
		final PhaseCorrelationPeak2 peaks = new PhaseCorrelationPeak2(new Point(new int[] {10, 10}), 1.0);
		final Dimensions pcmDims = new FinalDimensions(new int[] {50, 50});
		final Dimensions imgDims = new FinalDimensions(new int[] {30, 30});
