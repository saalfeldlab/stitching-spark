...
public class BlendedExtendedMirroredRandomAccesible2 <T extends RealType<T>>implements RandomAccessible<T> {
	...
	public BlendedExtendedMirroredRandomAccesible2(final RandomAccessibleInterval<T> img, final int[] border) {
		this.img = img;
		this.numDimensions = img.numDimensions();

		final float[] blendingBorder = new float[numDimensions];
		final float[] border2 = new float[numDimensions];
	...
	public class BlendedRandomAccess extends Point implements RandomAccess<T>
	{
		...
		public RandomAccess<T> copyRandomAccess() {
			final BlendedRandomAccess a = new BlendedRandomAccess();
			a.move(this);
			return a;
		}
	...
	public static void main(final String[] args) {
		...
		new ImageJ();
		final Img<FloatType> img1 = ImgLib2Util.openAs32Bit(new File("src/main/resources/img1.tif"));
		final long[] dims = new long[img1.numDimensions()];

		final BlendedExtendedMirroredRandomAccesible2<FloatType> ext = new BlendedExtendedMirroredRandomAccesible2<>(img1, new int[]{100, 100, 10});

		ext.getExtInterval().dimensions(dims);
		final Img<FloatType> img2 = ArrayImgs.floats(dims);
		...
		final Cursor<FloatType> c = img2.cursor();
		for (final FloatType e : Views.iterable(Views.interval(ext, ext.getExtInterval())))
		{
			c.fwd();
			c.get().set(e);
		}
