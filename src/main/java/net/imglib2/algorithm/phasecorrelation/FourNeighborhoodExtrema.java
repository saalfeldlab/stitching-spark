...
public class FourNeighborhoodExtrema
{
	 ...
	 */
	public static <T> ArrayList<T> merge(final List<List<T>> lists, final int maxN, final Comparator<T> compare){
		final ArrayList<T> res = new ArrayList<>();
		final int[] idxs = new int[lists.size()];
		final boolean[] hasMore = new boolean[lists.size()];
	 ...
	 */
	public static List<Interval> splitAlongLargestDimension(final Interval interval, long nSplits){
		...
		final long chunkSize = interval.dimension(splitDim) / nSplits;
		final long maxSplitDim = max[splitDim];
	...
	public static < T extends RealType< T > > ArrayList< Pair< Localizable, Double > > findMaxMT( final RandomAccessible< T > img, final Interval region, final int maxN , final ExecutorService service){


		final int nTasks = Runtime.getRuntime().availableProcessors() * 4;
		final List<Interval> intervals = splitAlongLargestDimension(region, nTasks);
		...
		final ArrayList< Pair< Localizable, Double > > res = merge(toMerge, maxN, new Comparator<Pair< Localizable, Double >>() {
...
A:		while ( c.hasNext() )
	...
	public static void main( final String[] args )
	{
		...
		*/
		final ArrayList< Double > list1 = new ArrayList< >();
		list1.add( 5.0 );
		list1.add( 2.0 );
		list1.add( 0.0 );
		final ArrayList< Double > list2 = new ArrayList< >();
		list2.add( 8.0 );
		list2.add( 3.0 );
		list2.add( 1.0 );

		final ArrayList<List<Double>> both = new ArrayList<>();
		both.add(list1);
		both.add(list2);

		final ArrayList<Double> res = merge(both, 12, new Comparator<Double>() {
		...
		final Interval toSplit = new FinalInterval(new long[] {20, 40, 70});

		final List<Interval> splits = splitAlongLargestDimension(toSplit, 5);
	...
	public static void printList( final ArrayList< Double > list )
	{
