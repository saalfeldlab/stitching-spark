...
public class PhaseCorrelation2 {
	...
	public static <T extends ComplexType<T>, S extends ComplexType<S>, R extends RealType<R>> void calculatePCMInPlace(
			final RandomAccessibleInterval<T> fft1, final RandomAccessibleInterval<S> fft2, final RandomAccessibleInterval<R> pcm, final ExecutorService service)
	{
	...
	public static <T extends ComplexType<T>, S extends ComplexType<S>, R extends RealType<R>> void calculatePCM(
			final RandomAccessibleInterval<T> fft1, final RandomAccessibleInterval<T> fft1Copy, final RandomAccessibleInterval<S> fft2, final RandomAccessibleInterval<S> fft2Copy, final RandomAccessibleInterval<R> pcm,
	...
	public static <T extends ComplexType<T>, S extends ComplexType<S>, R extends RealType<R>> RandomAccessibleInterval<R> calculatePCMInPlace(
			final RandomAccessibleInterval<T> fft1, final RandomAccessibleInterval<S> fft2, final ImgFactory<R> factory, final R type, final ExecutorService service){

		final long[] paddedDimensions = new long[fft1.numDimensions()];
		final long[] realSize = new long[fft1.numDimensions()];

		FFTMethods.dimensionsComplexToRealFast(fft1, paddedDimensions, realSize);
		final RandomAccessibleInterval<R> res = factory.create(realSize, type);
	...
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>, C extends ComplexType<C>> RandomAccessibleInterval<R> calculatePCM(
			final RandomAccessibleInterval<T> img1, final RandomAccessibleInterval<S> img2, final int[] extension,
			final ImgFactory<R> factory, final R type, final ImgFactory<C> fftFactory, final C fftType, final ExecutorService service){
		...
		final Dimensions extSize = PhaseCorrelation2Util.getExtendedSize(img1, img2, extension);
		final long[] paddedDimensions = new long[extSize.numDimensions()];
		final long[] fftSize = new long[extSize.numDimensions()];
		FFTMethods.dimensionsRealToComplexFast(extSize, paddedDimensions, fftSize);

		final RandomAccessibleInterval<C> fft1 = fftFactory.create(fftSize, fftType);
		final RandomAccessibleInterval<C> fft2 = fftFactory.create(fftSize, fftType);
		...
		final RandomAccessibleInterval<R> pcm = calculatePCMInPlace(fft1, fft2, factory, type, service);
		return pcm;
	 ...
	 */
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>, C extends ComplexType<C>> RandomAccessibleInterval<R> calculatePCM(
			final RandomAccessibleInterval<T> img1, final RandomAccessibleInterval<S> img2, final ImgFactory<R> factory, final R type,
			final ImgFactory<C> fftFactory, final C fftType, final ExecutorService service) {
	...
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>> PhaseCorrelationPeak2 getShift(
			final RandomAccessibleInterval<R> pcm, final RandomAccessibleInterval<T> img1, final RandomAccessibleInterval<S> img2, final int nHighestPeaks,
			final long minOverlap, final boolean subpixelAccuracy, final boolean interpolateSubpixel, final ExecutorService service)
	{
		System.out.println( "PCM" );
		final List<PhaseCorrelationPeak2> peaks = PhaseCorrelation2Util.getPCMMaxima(pcm, service, nHighestPeaks, subpixelAccuracy);
	...
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>> PhaseCorrelationPeak2 getShift(
			final RandomAccessibleInterval<R> pcm, final RandomAccessibleInterval<T> img1, final RandomAccessibleInterval<S> img2, final int nHighestPeaks,
			final long minOverlap, final boolean subpixelAccuracy, final ExecutorService service)
	{
	 ...
	 */
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>> PhaseCorrelationPeak2 getShift(
			final RandomAccessibleInterval<R> pcm, final RandomAccessibleInterval<T> img1, final RandomAccessibleInterval<S> img2)
	{
		final ExecutorService service = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
		final PhaseCorrelationPeak2 res = getShift(pcm, img1, img2, 5, 0, true, false, service);
		service.shutdown();
		return res;
	}
	...
	public static void main(final String[] args) {
		...
		final Img<FloatType> img1 = ImgLib2Util.openAs32Bit(new File("src/main/resources/img1singleplane.tif"));
		final Img<FloatType> img2 = ImgLib2Util.openAs32Bit(new File("src/main/resources/img2singleplane.tif"));

		final ExecutorService service = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());

		final RandomAccessibleInterval<FloatType> pcm = calculatePCM(img1, img2, new ArrayImgFactory<FloatType>(), new FloatType(),
				new ArrayImgFactory<ComplexFloatType>(), new ComplexFloatType(), service );

		final PhaseCorrelationPeak2 shiftPeak = getShift(pcm, img1, img2);

		final RandomAccessibleInterval<FloatType> res = PhaseCorrelation2Util.dummyFuse(img1, img2, shiftPeak,service);
